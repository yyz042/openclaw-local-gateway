...
面向 OpenClaw 的轻量本地网关：按请求复杂度自动分流到不同模型，并提供会话级路由、长上下文治理与轻量会话记忆。

## 项目优势

- **多模型智能分流**：按复杂度自动选择 SIMPLE / MEDIUM / COMPLEX / REASONING 档位并映射到不同后端；分档沿用 `@blockrun/clawrouter` 的 `route()` 的加权评分与规则升档。
- **会话级智能路由**：多轮固定档位、复杂任务升档、相似请求三次升档；简单追问可走轻量模型且不降级会话记忆。
- **长上下文治理**：超长消息截断、大请求压缩，确保路由与上游输入一致。
- **轻量会话记忆**：识别总结/回顾类 prompt 注入 session journal，从 assistant 回复提取关键动作。
- **OpenClaw 友好**：路由前清洗 OpenClaw 元信息，避免分档输入与转发输入不一致。
- **可运维可解释**：完整记录 tier、confidence、weighted score 推理依据及流式/tool-call 输出。
- **JSON 配置 + 热重载**：`router.config.json` 统一管理 tier→backend、关键词与阈值；`POST /reload` 无需重启。
- **生产级回退**：primary + fallback 链，上游失败时按 `retryStatuses` 自动切换后端。
...
在 `proposed_tier` 基础上，按内存会话（优先 `x-session-id` / `x-clawrouter-session-id`）做二次决策：

| 规则 | 说明 |
|------|------|
| session-pinned | 新分档不高于会话档位时沿用会话档位 |
| session-upgrade | 新分档更高时升档并更新会话 |
| simple-follow-up | 本次为 SIMPLE 但会话记忆更高档时走轻量模型，不降级会话 |
| three-strike-escalation | 相同 prompt 指纹累计 3 次再升一档 |
...
- **记录**：上游成功返回后从 assistant 文本提取关键动作（如「创建/修复/实现…」），每会话最多 20 条。
...
| `GATEWAY_DRY_RUN` | `1/true` 时仅返回路由结果 | — |
| `GATEWAY_DEDUP_WINDOW_MS` | 重复请求拦截窗口 | — |
